...
def main():
    ...
    # Antes nos conectamos al servidor tanto para atacante como para defensor
    socket_defensor = UConn.connect_to_server("127.0.0.1", 25001)
    socket_atacante = UConn.connect_to_server("127.0.0.1", 25002)
    ...
    while True:
        ...
        if recording_cam:
            ...
            if len(corners) != 0:
                centers_aruco = []
                for cor in corners:
                    center_ar = list(np.sum(cor, axis=0, dtype=np.int) // 4)
                    center_ar.append(0)
                    # enviamos el centro de nuestro marcador aruco a Unity
                    centers_aruco.append(center_ar)
                if para_defensor:
                    UConn.send_positions(centers_aruco[0:1], False)
                else para_atacante:
                    UConn.send_positions(centers_aruco[0:2], False)
